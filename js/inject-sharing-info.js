( function() {
	...
	var svgImageSize = 512; // Size of downloadable image in px
  ...
  function buildTitleContainer() {
	 ...
	 * @param {boolean} [params.downloadButton=false] True, if button shall be shown.
	 * @param {string} [params.selector='h5p-sharing-null'] Selector.
	 * @return {HTMLElement} Container.
	 */
	function buildContainer( params ) {
		var container, containerTitle, containerContent, contentField, contentFieldText, buttonCopy, buttonDownload;
		...
		params.downloadButton = params.downloadButton || false;
		params.downloadContentTitle = params.downloadContentTitle || '';
		...
		// Download button
		if ( '' !== params.content && params.downloadButton ) {
			// buttonCopy = document.createElement( 'button' );
			buttonDownload = document.createElement( 'a' );
			buttonDownload.classList.add( 'button' );
			buttonDownload.classList.add( 'h5p-sharing-button-copy' );
			buttonDownload.classList.add( params.selector );
			// buttonCopy.setAttribute( 'type', 'button' );
			buttonDownload.innerHTML = l10n.downloadSVG;
			buttonDownload.href = params.content.src;
			
			var filename = "QRCODE_" + params.downloadContentTitle;

			buttonDownload.download = filename;

			// Button should not get focus
			buttonDownload.addEventListener( 'focus', function() {
				this.blur();
			});

			containerContent.appendChild( buttonDownload );
		}
  ...
  document.onreadystatechange = function() {
    ...
    if ( 'complete' === document.readyState ) {
			...
			sharingBoxContainer.appendChild( buildContainer({
				...
				showButton: false,
				downloadButton: embedAllowed,
				downloadContentTitle: h5pContentData.metadata.title,
